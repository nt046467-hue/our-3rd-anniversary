...
import { RingCarousel } from "@/components/ring-carousel/RingCarousel";
import type { RingMemoryItem } from "@/components/ring-carousel/types";
...
const reasonsDustParticles = [
  { x: 6, y: 9, size: 2, driftX: 6, driftY: -3, delay: 0, duration: 18 },
  { x: 24, y: 20, size: 1.4, driftX: 10, driftY: -6, delay: 2200, duration: 20 },
  { x: 72, y: 12, size: 1.8, driftX: -8, driftY: -4, delay: 4200, duration: 22 },
  { x: 42, y: 32, size: 2.4, driftX: 7, driftY: -5, delay: 1200, duration: 19 },
  { x: 15, y: 48, size: 1.6, driftX: -9, driftY: -3, delay: 1600, duration: 21 },
  { x: 83, y: 28, size: 2.1, driftX: 9, driftY: -2, delay: 3400, duration: 18 },
  { x: 55, y: 58, size: 1.5, driftX: -7, driftY: -5, delay: 2600, duration: 23 },
  { x: 33, y: 68, size: 2.2, driftX: 8, driftY: -6, delay: 4800, duration: 19 },
  { x: 91, y: 72, size: 1.2, driftX: -10, driftY: -4, delay: 700, duration: 20 },
  { x: 12, y: 80, size: 1.8, driftX: 6, driftY: -5, delay: 5200, duration: 22 },
  { x: 64, y: 82, size: 2.0, driftX: -8, driftY: -2, delay: 3300, duration: 18 },
  { x: 44, y: 88, size: 1.3, driftX: 10, driftY: -4, delay: 4300, duration: 20 },
];
...
const memoryNotes: Record<number, string> = {
  1: "Standing in the warm glow of the day, your laughter catching the afternoon light. It's moments like these where time seems to stand completely still.",
  2: "Looking out at the world with you right beside me. No matter how breathtaking the scenery is, my favorite view will always be you.",
  3: "Unfiltered, unscripted, and entirely ourselves. The quiet glances between words are where our story speaks loudest.",
  5: "Even the simplest afternoon turns into an unforgettable memory when we're together. You bring magic to the quietest days.",
  6: "Holding onto this feeling and wishing we could pause the clock. Every moment with you feels like a chapter I want to read over and over.",
  7: "That gentle, effortless smile that warms up the entire room. Being with you brings a calm and tender joy that nothing else can match.",
  8: "Captured in time, wrapped in love and comfort. Look at us—two hearts making everyday moments look like pure magic.",
  9: "Through every season, every laugh, and every quiet day in between. Choosing you is the easiest decision I make every single morning.",
  10: "A fleeting video snippet capturing our genuine smiles and live joy. A moving memory that brings our happiness right back to life.",
  11: "A precious glance saved away in my favorite corner of memory. No matter where we go, this warm feeling stays locked inside my heart.",
  12: "Surrounded by warm golden hues as the day fades into evening. The sunset is beautiful, but your presence makes it unforgettable.",
  13: "That special look reserved only for the one who knows you best. Your smile turns every ordinary place into home.",
  14: "Just the two of us, creating our own cozy space away from the noise. In your arms, I've found my absolute favorite place to be.",
};

const ringMemories: RingMemoryItem[] = [
  { ...allMemories[0], note: memoryNotes[1] }, // photo01
  { ...allMemories[1], note: memoryNotes[2] }, // photo02
  { ...allMemories[2], note: memoryNotes[3] }, // photo03
  { ...allMemories[4], note: memoryNotes[5] }, // photo04
  { ...allMemories[5], note: memoryNotes[6] }, // photo05
  { ...allMemories[6], note: memoryNotes[7] }, // photo06
  { ...allMemories[7], note: memoryNotes[8] }, // photo07
  { ...allMemories[9], note: memoryNotes[10] }, // memoryVideo01 (1 video)
];
...
export default function App() {
  ...
  const handleVnPause = () => {
    const vn = vnRef.current;
    const isEnded = vn && isFinite(vn.duration) && vn.currentTime >= vn.duration - 0.2;
    if (isManualPauseRef.current || isEnded) {
      setVnPlaying(false);
  ...
  const handleVnPlaying = () => {
    if (!vnPlaying) setVnPlaying(true);
    fadeAudioVolume(0.09, 600);
    if (!vnAnimRef.current) {
      vnAnimRef.current = requestAnimationFrame(updateVnProgress);
    }
  };
  ...
  useEffect(() => {
    const listener = (event: KeyboardEvent) => {
      ...
      if (selected) {
        ...
        if ((event.key === "ArrowLeft" || event.key === "ArrowUp") && idx > 0) setSelected(sourceList[idx - 1]);
        if ((event.key === "ArrowRight" || event.key === "ArrowDown") && idx < sourceList.length - 1) setSelected(sourceList[idx + 1]);
      }
    };
  ...
  useEffect(() => {
    const section = reasonsRef.current;
    if (!section) return;
    const prefersReducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    ...
    const update = () => {
      ...
      const offset = Math.max(-10, Math.min(10, (rect.top / window.innerHeight) * 8));
      section.style.setProperty("--reasons-bg-offset", `${offset}px`);
    };
  ...
  return <main className="site-shell">
    ...
    <section id="home" className="scroll-section hero">
      ...
      <div className="hero-portrait">
        <ProtectedImage src={photo01} alt="A tender couple's photograph" fit="cover" />
    ...
    <section id="memories" ref={galleryRef} className={`scroll-section gallery ${galleryVisible ? "gallery-in-view" : ""}`}>
      ...
      <RingCarousel memories={ringMemories} onSelectMemory={(memory) => setSelected(memory)} />
            ...
            >
              ...
              <div className="polaroid-frame">
                {memory.type === "video" ? (
                  ...
                  <ProtectedImage src={memory.src} alt={memory.caption} fit="contain" />
                )}
              </div>
    ...
    <section id="reasons" ref={reasonsRef} className="scroll-section reasons">
      <div className="reasons-bg" aria-hidden="true" />
      <div className="reasons-dust" aria-hidden="true">
        {reasonsDustParticles.map((particle, i) => (
          <span
            key={i}
            style={{
              "--rx": `${particle.x}%`,
    ...
    <section id="path" className="scroll-section path">
      <div className="path-luxury-bg" aria-hidden="true">
        <div className="path-emerald-base" />
        <div className="path-moonlight" />
        <div className="path-tulip-far" />
        <div className="path-paper-grain" />
        <div className="path-vignette" />
        <div className="path-mist" />
        <div className="path-pollen">
          {Array.from({ length: 14 }, (_, i) => (
            <span key={i} className="path-pollen-particle" style={{ "--ppi": i } as CSSProperties} />
          ))}
        </div>
        <div className="path-dust">
          {Array.from({ length: 10 }, (_, i) => (
            <span key={i} className="path-dust-particle" style={{ "--pdi": i } as CSSProperties} />
          ))}
        </div>
        <div className="path-petals">
          {Array.from({ length: 5 }, (_, i) => (
            <span key={i} className="path-petal" style={{ "--ppti": i } as CSSProperties}>
              {i % 2 === 0 ? "🌸" : "💮"}
            </span>
          ))}
        </div>
    ...
    {selected && (() => {
      ...
      const handleTouchEnd = (e: React.TouchEvent) => {
        ...
        const diffX = touch.clientX - swipeTouchRef.current.startX;
        const diffY = touch.clientY - swipeTouchRef.current.startY;
        swipeTouchRef.current = null;
        // Only swipe if horizontal movement is dominant and > 50px
        if (Math.abs(diffX) > 50 && Math.abs(diffX) > Math.abs(diffY) * 1.5) {
          if (diffX < 0 && hasNext) {
            setSelected(sourceList[currentIdx + 1]);
      ...
      return (
        <div
          className="lightbox romantic-lightbox"
          role="dialog"
          aria-modal="true"
          ...
          <div className="lightbox-inner lb-romantic-inner" onClick={(e) => e.stopPropagation()}>
            <div className="lb-media-frame">
              ...
              {selected.type === "video" ? (
                ...
                <ProtectedImage src={selected.src} alt={selected.caption} fit="contain" maxHeightVh={78} />
              )}
            </div>
